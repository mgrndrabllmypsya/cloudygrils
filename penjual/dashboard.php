...
<!-- Font sama seperti index.php: Poppins + Lato -->
...
.sidebar-logo {
    ...
    background: rgba(255,254,254,.12);
}
...
.nav-section {
    ...
    padding: 18px 16px 8px; font-weight: 600;
}
.nav-item {
    font-family: var(--font-ui);
...
.nav-item.active {
    ...
    border-left: 3px solid #fff; padding-left: 15px;
}
...
.btn-logout {
    ...
    color: rgba(255,255,255,.85);
    transition: background .2s; width: 100%; letter-spacing: 0.01em;
}
...
.btn-toko {
    ...
    box-shadow:0 3px 12px rgba(212,84,127,.35); transition:opacity .2s;
}
...
.stat-value { font-family: var(--font-heading); font-size:clamp(17px,2.2vw,26px); font-weight:700; color:var(--text); line-height:1; margin-bottom:5px; }
...
.chart-header h3 { font-family: var(--font-heading); font-size:14px; font-weight:600; color:var(--text); }
...
.card-header a { font-family: var(--font-ui); font-size:12px; color:var(--accent); font-weight:500; }
...
td { font-family: var(--font-body); padding:11px 18px; font-size:13px; border-bottom:1px solid #FFE0EF; color:var(--text2); }
...
.produk-nama { font-family: var(--font-ui); font-size:13px; font-weight:500; color:var(--text); }
.produk-kat  { font-family: var(--font-body); font-size:11px; color:var(--muted); }
.empty { font-family: var(--font-body); text-align:center; padding:36px 20px; color:var(--muted); font-size:13px; }
...
.modal-head-title { font-family: var(--font-heading); font-size:16px; font-weight:700; color:var(--text); }
.modal-head-sub   { font-family: var(--font-body); font-size:11px; color:var(--muted); margin-top:1px; }
.btn-close-modal {
...
.ua-text  { font-family: var(--font-ui); font-size:13px; color:var(--accent); font-weight:600; margin-top:5px; }
.ua-hint  { font-family: var(--font-body); font-size:11px; color:var(--muted); margin-top:2px; }
...
.btn-save-m {
    ...
    transition:opacity .2s; display:flex; align-items:center; gap:7px;
...
.donut-center .total-num   { font-family: var(--font-heading); font-size:22px; font-weight:700; color:var(--text); line-height:1; }
.donut-center .total-label { font-family: var(--font-body); font-size:10px; color:var(--muted); text-transform:uppercase; letter-spacing:1px; margin-top:2px; }
...
.legend-name  { font-family: var(--font-body); color:var(--text2); }
.legend-pct   { font-family: var(--font-heading); font-weight:600; color:var(--muted); font-size:12px; }
...
@media (max-width:900px) {
    ...
    .sidebar {
        ...
        height: 100vh !important; width: 220px !important; max-width: 65vw !important;
        border-radius: 0 24px 24px 0 !important; overflow: hidden !important;
        display: flex !important; flex-direction: column !important;
        transform: translateX(-100%); transition: transform 0.3s ease; z-index: 99;
    }
    .sidebar.active { transform: translateX(0); }
    .sidebar-nav { flex: 1 !important; overflow-y: auto !important; scrollbar-width: none !important; }
                ...
                <div class="table-wrap">
                    ...
                    <table>
                        ...
                        <?php while ($row = mysqli_fetch_assoc($q_pesanan)): ?>
                            <tr>
                                <td>
                                    <div style="font-family:var(--font-ui);font-weight:500;color:var(--text);"><?= escape($row['nama_pembeli']) ?></div>
                                    <div style="font-family:var(--font-body);font-size:11px;color:var(--muted);"><?= date('d M Y', strtotime($row['created_at'])) ?></div>
                                </td>
                                <td style="font-family:var(--font-heading);font-weight:600;color:var(--accent);"><?= formatRupiah($row['total_bayar']) ?></td>
                                <td>
                                    <?php
                    ...
                    <table>
                        ...
                        <?php while ($row = mysqli_fetch_assoc($q_produk)): ?>
                            <tr>
                                ...
                                <td style="font-family:var(--font-heading);font-weight:600;color:var(--accent);"><?= formatRupiah($row['harga']) ?></td>
                                <td>
    ...
    <div class="modal-box">
        ...
        <form method="POST" action="produk.php" enctype="multipart/form-data">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <select name="ukuran" class="form-ctrl">
                                ...
                                <option>XXXL</option><option>All Size</option>
                            </select>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-foot">
                <button type="button" class="btn-cancel-m" onclick="closeTambahModal()"><i class="bi bi-x"></i> Batal</button>
                <button type="submit" class="btn-save-m"><i class="bi bi-floppy"></i> Simpan Produk</button>
            </div>
        </form>
    </div>
</div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (toggleBtn && sidebar) {
        toggleBtn.addEventListener('click', function(e) { sidebar.classList.toggle('active'); e.stopPropagation(); });
...
function buildChart(period) {
    ...
    salesChart = new Chart(ctx, {
        type: 'line',
        data: { labels, datasets: [{ label: 'Penjualan (Rp)', data: values, borderColor: '#E8719A', backgroundColor: grad, borderWidth: 2.5, pointBackgroundColor: '#E8719A', pointBorderColor: '#fff', pointBorderWidth: 2, pointRadius: 4, pointHoverRadius: 6, fill: true, tension: 0.4 }] },
        options: {
            responsive: true, maintainAspectRatio: false,
            plugins: { legend: { display: false }, tooltip: { backgroundColor: '#fff', borderColor: '#F4A7C3', borderWidth: 1.5, titleColor: '#1A1A1A', bodyColor: '#E8719A', padding: 10, callbacks: { label: ctx => ' Rp ' + ctx.parsed.y.toLocaleString('id-ID') } } },
            scales: {
                x: { grid: { color: 'rgba(212,167,195,0.3)' }, ticks: { color: '#AAAAAA', font: { size: 11, family: "'Lato', sans-serif" } } },
                y: { grid: { color: 'rgba(212,167,195,0.3)' }, ticks: { color: '#AAAAAA', font: { size: 11, family: "'Lato', sans-serif" }, callback: v => 'Rp ' + (v >= 1000000 ? (v/1000000).toFixed(1)+'jt' : v >= 1000 ? (v/1000).toFixed(0)+'rb' : v) }, beginAtZero: true }
            }
        }
    });
}
...
(function() {
    ...
    new Chart(ctx, { type: 'doughnut', data: { labels, datasets: [{ data: values, backgroundColor: colors.slice(0, values.length), borderWidth: 3, borderColor: '#fff', hoverBorderWidth: 3, hoverOffset: 6 }] }, options: { responsive: true, maintainAspectRatio: true, cutout: '68%', plugins: { legend: { display: false }, tooltip: { backgroundColor: '#fff', borderColor: '#F4A7C3', borderWidth: 1.5, titleColor: '#1A1A1A', bodyColor: '#E8719A', padding: 10, callbacks: { label: ctx => ' ' + ctx.label + ': ' + ctx.parsed + ' terjual' } } } } });
})();
<?php endif; ?>

function openTambahModal()  { document.getElementById('dashModalOverlay').classList.add('show'); document.body.style.overflow = 'hidden'; }
...
document.getElementById('dashModalOverlay').addEventListener('click', function(e) { if (e.target === this) closeTambahModal(); });
</script>
</body>
</html>